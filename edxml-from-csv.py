...
DateAcquired = "%04d%02d%02d" % (
    (TimeAcquired.tm_year, TimeAcquired.tm_mon, TimeAcquired.tm_mday))
...
for Line in Input:
    if ColumnPropertyMapping is None:
        ...
        PropertyColumnMapping = {Property: Index for Index, Property in dict(
            enumerate(ColumnPropertyMapping)).items()}
    ...
    if OutputEventType not in OutputProperties:
        OutputProperties[OutputEventType] = Parser.getOntology(
        ).GetEventType(OutputEventType).GetProperties()
        UniqueOutputProperties[OutputEventType] = Parser.getOntology(
        ).GetEventType(OutputEventType).GetUniqueProperties()

        OutputProperties[OutputEventType] = [
            Property for Property in OutputProperties[OutputEventType] if Property in PropertyColumnMapping]
        UniqueOutputProperties[OutputEventType] = [
            Property for Property in UniqueOutputProperties[OutputEventType] if Property in PropertyColumnMapping]
    ...
    for Property in OutputProperties[OutputEventType]:
        if SplitLine[PropertyColumnMapping[Property]] != '':
            LineProperties[Property] = [
                SplitLine[PropertyColumnMapping[Property]]]
    ...
    if PreviousSplitLine is not None:
        ...
        if len(Difference) == 2:
            ...
            # is different.
            DiffProperties = [
                Property for Property in LineProperties if LineProperties[Property] != PreviousLineProperties[Property]]
            ...
            if len(DiffProperties) > 0 and DiffProperties[0] not in UniqueOutputProperties[OutputEventType]:
                ...
                PreviousLineProperties[Property].extend(
                    LineProperties[Property])
                continue
        ...
        if OutputEventType != CurrentOutputEventType:
            if CurrentOutputEventType is not None:
                if PreviousLineProperties is not None:
                    try:
                        MyWriter.AddEvent(
                            OutputEvent.SetProperties(PreviousLineProperties))
...
if PreviousSplitLine is not None:
    if OutputEventType != CurrentOutputEventType:
        ...
        if OutputEventType not in OutputProperties:
            ...
            OutputProperties[OutputEventType] = [
                p for p in OutputProperties if p in PropertyColumnMapping]
            UniqueOutputProperties[OutputEventType] = [
                p for p in UniqueOutputProperties if p in PropertyColumnMapping]
