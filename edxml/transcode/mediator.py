...
class TranscoderMediator(object):
    ...
    def describe(self, source_name):

        if self._last_written_ontology_version == 0:
            self._write_initial_ontology()

        # TODO: Describe parent / child hierarchy?
        # TODO: Describe event type attachments?

        description = f"\n\nThis transcoder reads {source_name} and outputs "\
                      f"`EDXML <http://edxml.org/>`_ containing {self._describe_event_types()}.\n"

        relates_inter = self._describe_inter_concept_relations()
        relates_intra = self._describe_intra_concept_relations()
        ...
        for source_concept_name, target_concept_names in relates_intra.items():
            for target_concept_name, object_type_names in target_concept_names.items():
                ...
                for object_type_name in object_type_names:
                    object_type_dn.append(self._ontology.get_object_type(object_type_name).get_display_name_plural())
                description += ', '.join(object_type_dn)

        if concept_refinements:
            description += '\nThe transcoder identifies\n' + '\n'.join(concept_refinements)

        concepts = self._describe_concepts()
        object_types = self._describe_object_types()
    ...
    def _describe_concepts(self):
        concept_names = []
        for concept in self._ontology.get_concepts().values():
            concept_names.append(f"{concept.get_name()} ({concept.get_display_name_singular()})")
        return concept_names

    def _describe_object_types(self):
        object_type_names = []
        for object_type in self._ontology.get_object_types().values():
            object_type_names.append(f"{object_type.get_name()} ({object_type.get_display_name_singular()})")
        return object_type_names

    def _describe_event_types(self):
        type_names = []
        for event_type_name in self._ontology.get_event_type_names():
            type_names.append(self._ontology.get_event_type(event_type_name).get_display_name_plural())
        if len(type_names) > 1:
            last = type_names.pop()
    ...
    def _describe_inter_concept_relations(self):
        relations = defaultdict(lambda: defaultdict(set))
        for event_type_name in self._ontology.get_event_type_names():
            event_type = self._ontology.get_event_type(event_type_name)
    ...
    def _describe_intra_concept_relations(self):
        relations = defaultdict(lambda: defaultdict(set))
        for event_type_name in self._ontology.get_event_type_names():
            event_type = self._ontology.get_event_type(event_type_name)
            for relation in event_type.get_property_relations().values():  # type: PropertyRelation
    ...
    def _describe_concept_refinements(self, item_template):
        descriptions = []
        for event_type_name in self._ontology.get_event_type_names():
            event_type = self._ontology.get_event_type(event_type_name)
            for relation in event_type.get_property_relations().values():  # type: PropertyRelation
                if relation.get_type() != 'intra':
                    continue
                source_concept = self._ontology.get_concept(relation.get_source_concept())
                target_concept = self._ontology.get_concept(relation.get_target_concept())
