  ...
  def _StripControlChars(Context, Strings):
    ...
    if Strings:
      ...
      for String in Strings:
        OutStrings.append("".join(ch for ch in unicode(String) if unicodedata.category(ch)[0] != "C"))
    ...
    for XPath, PropertyName in self.XPATH_MAP.items():
      ...
      if XPath not in XmlTranscoder._XPATH_MATCHERS:
        ...
        try:
          ...
          raise ValueError(
            'TYPE_MAP of %s contains invalid XPath for property %s: %s' % (self.__class__, PropertyName, XPath)
          )
