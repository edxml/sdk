...
class XmlTranscoderMediator(TranscoderMediator):
    ...
    def Parse(self, inputFile, tags, attribute_defaults=False, dtd_validation=False, load_dtd=False, no_network=True,
        ...
        elementIterator = etree.iterparse(
            inputFile, events=['end'], tag=tags, attribute_defaults=attribute_defaults, dtd_validation=dtd_validation,
    ...
    def Generate(self, inputFile, tags, attribute_defaults=False, dtd_validation=False, load_dtd=False, no_network=True,
                 remove_blank_text=False, remove_comments=False, remove_pis=False, encoding=None, html=False,
                 recover=None, huge_tree=False, schema=None, resolve_entities=False):
        """
    ...
    def Process(self, Element, Tree=None):
        ...
        TranscoderPaths = sorted(XmlTranscoderMediator._XPATH_MATCHERS.keys(
        ), key=lambda xpath: ElementXPath.startswith(xpath))
                ...
                if self._transcoderIsPostprocessor(Transcoder):
                    try:
                        ...
                        self.Warning(('The post processor of the transcoder for XML element at %s failed '
                                      'with %s: %s\n\nContinuing...') % (
                                          ElementXPath, type(Except).__name__, str(Except))
                                     )
                else:
                    try:
