...
def test_validate_datetime():
    ...
    with pytest.raises(EDXMLEventValidationError, match='Invalid value'):
        DataType.datetime().validate_object_value('1978-06-17')

    with pytest.raises(EDXMLEventValidationError, match="cannot be used to store values of type 'int'"):
...
def test_validate_number_integer():
    ...
    for integer_type in integer_types:
        ...
        with pytest.raises(EDXMLEventValidationError, match="cannot be used to store values of type 'float'"):
            integer_type.validate_object_value(0.5)
        with pytest.raises(EDXMLEventValidationError, match="cannot be used to store values of type 'bool'"):
...
def test_validate_decimal():
    ...
    with pytest.raises(EDXMLEventValidationError, match='Must have 2 fractional digits'):
        decimal.validate_object_value('1')
    with pytest.raises(EDXMLEventValidationError, match='Must have 2 fractional digits'):
        decimal.validate_object_value('1.0')
    with pytest.raises(EDXMLEventValidationError, match='Zero padding of the integral part is not allowed'):
        decimal.validate_object_value('01.00')
    with pytest.raises(EDXMLEventValidationError, match='Zero must not have any sign'):
...
def test_validate_currency():
    ...
    with pytest.raises(EDXMLEventValidationError, match='Must have four fractional digits'):
...
def test_validate_hex():
    ...
    with pytest.raises(EDXMLEventValidationError, match='Must be all lowercase'):
        DataType.hex(1).validate_object_value('AA')
    with pytest.raises(EDXMLEventValidationError, match='Incorrect length'):
...
def test_validate_ip():
    ...
    with pytest.raises(EDXMLEventValidationError, match='IP addresses must not be shortened or zero padded'):
...
def test_validate_geo():
    ...
    with pytest.raises(EDXMLEventValidationError, match='Missing latitude decimals'):
        DataType.geo_point().validate_object_value('-8.2,25.300000')
    with pytest.raises(EDXMLEventValidationError, match='Missing longitude decimals'):
        DataType.geo_point().validate_object_value('-8.200000,25.3')
    with pytest.raises(EDXMLEventValidationError, match=r"Longitude has a leading '\+' sign"):
        DataType.geo_point().validate_object_value('-8.200000,+25.300000')
    with pytest.raises(EDXMLEventValidationError, match=r"Latitude has a leading '\+' sign"):
        DataType.geo_point().validate_object_value('+8.200000,25.300000')
    with pytest.raises(EDXMLEventValidationError, match=r'Latitude not within range \[-90,90\]'):
        DataType.geo_point().validate_object_value('91.200000,25.300000')
    with pytest.raises(EDXMLEventValidationError, match=r"Longitude not within range \[-180,180\]"):
...
def test_validate_string():
    ...
    with pytest.raises(EDXMLEventValidationError, match='Empty object values are not valid'):
        DataType.string().validate_object_value('')
    with pytest.raises(EDXMLEventValidationError, match='String is too long'):
        DataType.string(length=1).validate_object_value('foo')
    with pytest.raises(EDXMLEventValidationError, match='String contains non-latin1 characters'):
        DataType.string(require_unicode=False).validate_object_value('😀')
    with pytest.raises(EDXMLEventValidationError, match='String must be all lowercase'):
        DataType.string(upper_case=False).validate_object_value('A')
    with pytest.raises(EDXMLEventValidationError, match='String must be all uppercase'):
...
def test_validate_sequence():
    ...
    with pytest.raises(EDXMLEventValidationError, match="cannot be negative"):
